...
export type RemoteProperty = {
	_remoteSignal: RemoteSignal.RemoteSignal,
	_value: any,
	_perPlayer: { Player: any },
	_playerRemoving: RBXScriptConnection,
}

function RemoteProperty.new(parent: Instance, name: string, initialValue: any) : RemoteProperty
	local self = setmetatable({} :: RemoteProperty, RemoteProperty)
	self._remoteSignal = RemoteSignal.new(parent, name, false)
	self._value = initialValue
	self._perPlayer = {}
	self._playerRemoving = Players.PlayerRemoving:Connect(function(player)
		self._perPlayer[player] = nil
	end)
	self._remoteSignal:Connect(function(player)
		...
		self._remoteSignal:Fire(player, value)
	end)
	return self
end
...
function RemoteProperty.Set(self : RemoteProperty, value: any)
	self._value = value
	table.clear(self._perPlayer)
	self._remoteSignal:FireAll(value)
end
...
function RemoteProperty.SetTop(self : RemoteProperty, value: any)
	self._value = value
...
function RemoteProperty.SetFilter(self : RemoteProperty, predicate: (Player, any) -> boolean, value: any)
	for _, player in ipairs(Players:GetPlayers()) do
...
function RemoteProperty.SetFor(self : RemoteProperty, player: Player, value: any)
...
function RemoteProperty.SetForList(self : RemoteProperty, players: { Player }, value: any)
...
function RemoteProperty.ClearFor(self : RemoteProperty, player: Player)
	if self._perPlayer[player] == nil then
		return
	end
	self._perPlayer[player] = nil
	self._remoteSignal:Fire(player, self._value)
end
...
function RemoteProperty.ClearForList(self : RemoteProperty, players: { Player })
	for _, player in ipairs(players) do
...
function RemoteProperty.ClearFilter(self : RemoteProperty, predicate: (Player) -> boolean)
	for _, player in ipairs(Players:GetPlayers()) do
...
function RemoteProperty.Get(self : RemoteProperty): any
	return self._value
end
...
function RemoteProperty.GetFor(self : RemoteProperty, player: Player): any
	local playerValue = self._perPlayer[player]
	local value = if playerValue == nil then self._value elseif playerValue == None then nil else playerValue
	return value
end
...
function RemoteProperty.Destroy(self : RemoteProperty)
	self._remoteSignal:Destroy()
